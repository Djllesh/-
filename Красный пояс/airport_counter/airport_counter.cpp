...
public:
    ...
    AirportCounter(TIterator begin, TIterator end)
    {
        while (begin != end)
        {
            ++num_airports[static_cast<uint32_t>(*begin)];
            begin++;
        }
    }
    ...
    {
        return num_airports.at(static_cast<uint32_t>(airport));
    }

    // добавить данный элемент
    void Insert(TAirport airport)
    {
        ++num_airports[static_cast<uint32_t>(airport)];
    }
    ...
    {
        num_airports[static_cast<uint32_t>(airport)] = 0;
    }
    ...
    Items GetItems() const
    {
        ...
        {
            result[i] = make_pair(static_cast<TAirport>(i), num_airports.at(i));
        }
        return result;
    }

private:    
    array<size_t, N> num_airports = { 0 };
